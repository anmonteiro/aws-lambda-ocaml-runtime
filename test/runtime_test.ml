...
let suite =
  [ ( "successful handler invocation"
    , `Quick
    , fun () ->
        let handler _event _ctx = Ok (`String "Hello") in
  ...
  ; ( "simple asynchronous handler invocation"
    , `Quick
    , fun () ->
        let handler _event _ctx = Lwt_result.return (`String "Hello") in
  ...
  ; ( "failed handler invocation"
    , `Quick
    , fun () ->
        let handler _event _ctx = raise User_code_exception in
        test_runtime handler (fun output ->
            match output with
            | Ok result ->
              let result_str = Yojson.Safe.to_string result in
